...
-- CTE: aggregate clickstream metrics per student in the first 28 days.
-- The NTILE window function is applied in the outer SELECT over the
-- pre-aggregated total_clicks_first_28 column, avoiding the non-portable
-- pattern of mixing aggregates inside window ORDER BY (which BigQuery
-- and strict ANSI engines reject).
...
SELECT
    ...
    sa.active_days_first_28,
    sa.total_clicks_first_28,
    sa.avg_clicks_per_active_day,
    sa.last_active_day_in_window,
    ...
    NTILE(10) OVER (
        PARTITION BY sa.code_module, sa.code_presentation
        ORDER BY sa.total_clicks_first_28
